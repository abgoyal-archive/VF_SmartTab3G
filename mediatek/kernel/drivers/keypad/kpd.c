...
#define KPD_PWRKEY_USE_HALL_EINT KPD_YES  //added by zhengdao
#define CUST_EINT_POLARITY_LOW              0   //added by zhengdao


//extern void mt_eint_hall_register(void);
...
/***********************************/
static u8 kpd_pwrkey_hall_state = !CUST_EINT_POLARITY_LOW;  //added by zhengdao,for give a initial value
...
#if (KPD_PWRKEY_USE_EINT||KPD_PWRKEY_USE_HALL_EINT)     //changed by zhengdao,make the two function run

 void kpd_pwrkey_handler_hal(unsigned long data);


 void kpd_pwrkey_handler(unsigned long data);
	  

DECLARE_TASKLET(kpd_pwrkey_tasklet, kpd_pwrkey_handler, 0);
...
#if (KPD_PWRKEY_USE_EINT||KPD_PWRKEY_USE_HALL_EINT)  //added by zhengdao,make the function run
 void kpd_pwrkey_handler(unsigned long data)
{
	kpd_pwrkey_handler_hal(data);
}

 void kpd_pwrkey_eint_handler(void)
{
	tasklet_schedule(&kpd_pwrkey_tasklet);
	printk("kpd_pwrkey_eint_handler");
}
#endif


/******** the main work function ,simulate the event of power key ******/
bool hall_covered = 0;
void kpd_pwrkey_handler_hal(unsigned long data){

	bool pressed;
	u8 old_state = kpd_pwrkey_hall_state;

	mt_eint_mask(CUST_EINT_MHALL_NUM);
	
	printk("kpd_pwrkey_handler_hal");

	kpd_pwrkey_hall_state = !kpd_pwrkey_hall_state;
	pressed = (kpd_pwrkey_hall_state == !!CUST_EINT_POLARITY_LOW);
...
/*******************wh@tcl 2014-1-3***********************************/
static int hall_status_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    len += sprintf(page, "%d\n", hall_covered);
    return len;
...
#if KPD_PWRKEY_USE_HALL_EINT   //added by zhengdao

#ifndef  PIXO_7_3G      //added by chenweihong PIXO_7_G has no hall
//mt_eint_hall_register();
...
//mt65xx_eint_registration(CUST_EINT_MHALL_NUM, true, CUST_EINT_POLARITY_LOW,
...
mt_eint_registration(CUST_EINT_MHALL_NUM, CUST_EINT_MHALL_TYPE, kpd_pwrkey_eint_handler, 0); 
mt_eint_unmask(CUST_EINT_MHALL_NUM);
...
static struct proc_dir_entry *hall_dir,*hall_proc;
...
hall_dir = proc_mkdir("hall",proc_dir);
...
#if defined(KPD_PWRKEY_USE_HALL_EINT)
#ifndef  PIXO_7_3G      //added by chenweihong PIXO_7_G has no hall
	    hall_proc = create_proc_entry( "hall", 0666, hall_dir);
    ...
    {
        hall_proc->read_proc = NULL;
        hall_proc->write_proc = NULL;
    }
#endif
#endif
	create_proc_read_entry("hall_status", 0, NULL, hall_status_read_proc, NULL);
	return 0;
}
