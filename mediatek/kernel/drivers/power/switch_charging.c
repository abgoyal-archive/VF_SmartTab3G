...
#define FULL_CHECK_TIMES		6
 ...
 //extern function
 // ============================================================ //
 extern int get_lcd_backlight_on_status(void);
...
void select_charging_curret(void)
{
    ...
    {    
		...
		#ifdef CHARGE_CURRENT_LIMIT	/* allan add start on 20140124 */
			if (get_lcd_backlight_on_status() == true)
				g_temp_CC_value = LCD_ON_STANDARD_CHARGER_CURRENT;
			else
				g_temp_CC_value = STANDARD_CHARGER_CURRENT;
		#else
			g_temp_CC_value = STANDARD_CHARGER_CURRENT;	//AC_CHARGER_CURRENT; 
		#endif
        }
...
void select_temp_limit_charging_curret(void)	/* allan modify on 20131024 */
{
    ...
    else
    {
        ...
        {    
			...
			/*Set CV Voltage*/
                    ...
                    cv_voltage = BATTERY_VOLT_04_200000_V;
                #endif   
				/* allan modify start on 20131224 */
				if ((BMT_status.temperature > MAX_LIMIT_CHARGE_TEMPERATURE) && (BMT_status.temperature < MAX_CHARGE_TEMPERATURE) )
				{
					cv_voltage = BATTERY_VOLT_04_100000_V;
				}
		  		/* allan modify end 20131224 */
