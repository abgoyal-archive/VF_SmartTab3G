...
/* allan add start on 20131122 */
#define BATTERY_COMPATIBLE	1	
#ifdef BATTERY_COMPATIBLE
typedef enum
{
	BAT_FEIMT_POP7,
	BAT_MBELL_POP7,
...
kal_int32 battery_compatible = BAT_OTHER_POP7;
int bat_id_vol = 0;
#endif
/* allan add end on 20131122 */

/* allan add end on 20131122 */
kal_int32 suspend_current = DEFAUL_SUSPEND_CURRENT;
kal_int32 g_sleep_fg_soc = 0;
kal_int32 g_sleep_fg_ui_soc = 0;
kal_int32 resume_count = 0;

/* [PLATFORM]-Add-BEGIN by TCTSZ.pingao.yang, 2014.9.3, refer pr 781992 */
#define CURRENT_ADJUST_MUL 100
#define CURRENT_ADJUST_DIV 124
/* [PLATFORM]-Add-END by TCTSZ.pingao.yang, 2014.9.3 */
...
kal_int32 fgauge_get_Q_max(kal_int16 temperature)
{
    ...
    if (temperature <= TEMPERATURE_T1)
    {
	...
	#ifdef BATTERY_COMPATIBLE	/* allan add start on 20131122 */
		if((battery_compatible == BAT_FEIMT_POP8) || (battery_compatible == BAT_FEIMT_POP7))
		{
			low_Q_max = FEIMT_Q_MAX_NEG_10;
        	high_Q_max = FEIMT_Q_MAX_POS_0;
		}
		else if((battery_compatible == BAT_MBELL_POP8) || (battery_compatible == BAT_MBELL_POP7))
		{
			low_Q_max = MBELL_Q_MAX_NEG_10;
        	high_Q_max = MBELL_Q_MAX_POS_0;
		}
		else
		{
			low_Q_max = Q_MAX_NEG_10;
		...
		low_Q_max = Q_MAX_NEG_10;
        high_Q_max = Q_MAX_POS_0;
	#endif	/* allan add end on 20131122 */
    ...
    else if (temperature <= TEMPERATURE_T2)
    {
		...
		if((battery_compatible == BAT_FEIMT_POP8) || (battery_compatible == BAT_FEIMT_POP7))
		{
			low_Q_max = FEIMT_Q_MAX_POS_0;
        	high_Q_max = FEIMT_Q_MAX_POS_25;
		}	
		else if((battery_compatible == BAT_MBELL_POP8) || (battery_compatible == BAT_MBELL_POP7))
		{
			low_Q_max = MBELL_Q_MAX_POS_0;
        	high_Q_max = MBELL_Q_MAX_POS_25;
		}
		else
		{
			low_Q_max = Q_MAX_POS_0;
    ...
    else
    {
		...
		if((battery_compatible == BAT_FEIMT_POP8) || (battery_compatible == BAT_FEIMT_POP7))
		{
			low_Q_max = FEIMT_Q_MAX_POS_25;
    		high_Q_max = FEIMT_Q_MAX_POS_50;
		}	
		else if((battery_compatible == BAT_MBELL_POP8) || (battery_compatible == BAT_MBELL_POP7))
		{
			low_Q_max = MBELL_Q_MAX_POS_25;
    		high_Q_max = MBELL_Q_MAX_POS_50;
		}	
		else
		{
			low_Q_max = Q_MAX_POS_25;
...
kal_int32 fgauge_get_Q_max_high_current(kal_int16 temperature)
{
    ...
    if (temperature <= TEMPERATURE_T1)
    {
		...
		if((battery_compatible == BAT_FEIMT_POP8) || (battery_compatible == BAT_FEIMT_POP7))
		{
			low_Q_max = FEIMT_Q_MAX_NEG_10_H_CURRENT;
        	high_Q_max = FEIMT_Q_MAX_POS_0_H_CURRENT;
		}	
		else if((battery_compatible == BAT_MBELL_POP8) || (battery_compatible == BAT_MBELL_POP7))
		{
			low_Q_max = MBELL_Q_MAX_NEG_10_H_CURRENT;
        	high_Q_max = MBELL_Q_MAX_POS_0_H_CURRENT;
		}	
		else
		{
			low_Q_max = Q_MAX_NEG_10_H_CURRENT;
    ...
    else if (temperature <= TEMPERATURE_T2)
    {
		...
		if((battery_compatible == BAT_FEIMT_POP8) || (battery_compatible == BAT_FEIMT_POP7))
		{
			low_Q_max = FEIMT_Q_MAX_POS_0_H_CURRENT;
        	high_Q_max = FEIMT_Q_MAX_POS_25_H_CURRENT;
		}	
		else if((battery_compatible == BAT_MBELL_POP8) || (battery_compatible == BAT_MBELL_POP7))
		{
			low_Q_max = MBELL_Q_MAX_POS_0_H_CURRENT;
        	high_Q_max = MBELL_Q_MAX_POS_25_H_CURRENT;
		}	
		else
		{
    ...
    else
    {
		...
		if((battery_compatible == BAT_FEIMT_POP8) || (battery_compatible == BAT_FEIMT_POP7))
		{
			low_Q_max = FEIMT_Q_MAX_POS_25_H_CURRENT;
        	high_Q_max = FEIMT_Q_MAX_POS_50_H_CURRENT;
		}	
		else if((battery_compatible == BAT_MBELL_POP8) || (battery_compatible == BAT_MBELL_POP7))
		{
			low_Q_max = MBELL_Q_MAX_POS_25_H_CURRENT;
        	high_Q_max = MBELL_Q_MAX_POS_50_H_CURRENT;
		}	
		else
		{
			low_Q_max = Q_MAX_POS_25_H_CURRENT;
...
{
#ifdef BATTERY_COMPATIBLE	/* allan add start on 20131122 */
	#ifdef POP7_BATTERY_COMPATIBLE	//pop7
		if(battery_compatible == BAT_FEIMT_POP7)
	    	return sizeof(feimt_battery_profile_t2) / sizeof(BATTERY_PROFILE_STRUC);
		else if(battery_compatible == BAT_MBELL_POP7)
			return sizeof(mbell_battery_profile_t2) / sizeof(BATTERY_PROFILE_STRUC);
		else
			return sizeof(battery_profile_t2) / sizeof(BATTERY_PROFILE_STRUC);
	#else	//pop8
		if(battery_compatible == BAT_FEIMT_POP8)
	    	return sizeof(feimt_battery_profile_t2) / sizeof(BATTERY_PROFILE_STRUC);
		else
			return sizeof(battery_profile_t2) / sizeof(BATTERY_PROFILE_STRUC);
	#endif
...
int fgauge_get_saddles_r_table(void)
{
	...
	#ifdef POP7_BATTERY_COMPATIBLE	//pop7
			if(battery_compatible == BAT_FEIMT_POP7)
				return sizeof(feimt_r_profile_t2) / sizeof(R_PROFILE_STRUC);
			else if(battery_compatible == BAT_MBELL_POP7)
				return sizeof(mbell_r_profile_t2) / sizeof(R_PROFILE_STRUC);
			else
...
BATTERY_PROFILE_STRUC_P fgauge_get_profile(kal_uint32 temperature)
{
    ...
    {	
	        ...
	        case TEMPERATURE_T0:	
				if(battery_compatible == BAT_FEIMT_POP7)
				{
		            return &feimt_battery_profile_t0[0];
		            break;    
				}
				else if(battery_compatible == BAT_MBELL_POP7)
				{
					return &mbell_battery_profile_t0[0];
		            break;   
				}
				else
				{
	        ...
	        case TEMPERATURE_T1:
	            if(battery_compatible == BAT_FEIMT_POP7)
				{
		            return &feimt_battery_profile_t1[0];
		            break;    
				}
				else if(battery_compatible == BAT_MBELL_POP7)
				{
					return &mbell_battery_profile_t1[0];
		            break;   
				}
				else
				{
	        ...
	        case TEMPERATURE_T2:
				if(battery_compatible == BAT_FEIMT_POP7)
				{
		            return &feimt_battery_profile_t2[0];
		            break;    
				}
				else if(battery_compatible == BAT_MBELL_POP7)
				{
					return &mbell_battery_profile_t2[0];
		            break;   
				}
				else
				{
	        ...
	        case TEMPERATURE_T3:
				if(battery_compatible == BAT_FEIMT_POP7)
				{
		            return &feimt_battery_profile_t3[0];
		            break;    
				}
				else if(battery_compatible == BAT_MBELL_POP7)
				{
					return &mbell_battery_profile_t3[0];
		            break;   
				}
				else
				{
	        ...
	        case TEMPERATURE_T:
				if(battery_compatible == BAT_FEIMT_POP7)
				{
		            return &feimt_battery_profile_temperature[0];
		            break;    
				}
				else if(battery_compatible == BAT_MBELL_POP7)
				{
					return &mbell_battery_profile_temperature[0];
		            break;   
				}
				else
				{
...
R_PROFILE_STRUC_P fgauge_get_profile_r_table(kal_uint32 temperature)
{
    ...
    {      
			...
			case TEMPERATURE_T0:
				if(battery_compatible == BAT_FEIMT_POP7)
				{
					return &feimt_r_profile_t0[0];
					break;	  
				}
				else if(battery_compatible == BAT_MBELL_POP7)
				{
					return &mbell_r_profile_t0[0];
					break;	 
				}
				else
				{
			...
			case TEMPERATURE_T1:
				if(battery_compatible == BAT_FEIMT_POP7)
				{
					return &feimt_r_profile_t1[0];
					break;	  
				}
				else if(battery_compatible == BAT_MBELL_POP7)
				{
					return &mbell_r_profile_t1[0];
					break;	 
				}
				else
				{
			...
			case TEMPERATURE_T2:
				if(battery_compatible == BAT_FEIMT_POP7)
				{
					return &feimt_r_profile_t2[0];
					break;	  
				}
				else if(battery_compatible == BAT_MBELL_POP7)
				{
					return &mbell_r_profile_t2[0];
					break;	 
				}
				else
				{
			...
			case TEMPERATURE_T3:
				if(battery_compatible == BAT_FEIMT_POP7)
				{
					return &feimt_r_profile_t3[0];
					break;	  
				}
				else if(battery_compatible == BAT_MBELL_POP7)
				{
					return &mbell_r_profile_t3[0];
					break;	 
				}
				else
				{
			...
			case TEMPERATURE_T:
				if(battery_compatible == BAT_FEIMT_POP7)
				{
					return &feimt_r_profile_temperature[0];
					break;	  
				}
				else if(battery_compatible == BAT_MBELL_POP7)
				{
					return &mbell_r_profile_temperature[0];
					break;	 
				}
				else
				{
...
void dod_init(void)
{
	...
	/* allan add start on 20131217 */
	if(get_boot_mode()== RECOVERY_BOOT){
		if(g_rtc_fg_soc!=0){
			gFG_capacity_by_v = g_rtc_fg_soc;
			bm_print(BM_LOG_CRTI, "[FGADC] recovery_boot get battery capacity for rtc, gFG_capacity_by_v=%d\n", 
            gFG_capacity_by_v);
		}
	}
	/* allan add end on 20131217 */
...
void oam_init(void)
{
	...
	/* [PLATFORM]-Add-BEGIN by TCTSZ.pingao.yang, 2014.9.24, refer pr 795806 */
	if((bat_is_charger_exist() == KAL_TRUE) && ((gFG_voltage - vol_bat) > POWER_HW_OCV_VBAT_DIFF))
	{
		gFG_voltage = vol_bat - POWER_HW_OCV_FLAOT_VOL;
	}
	/* [PLATFORM]-Add-END by TCTSZ.pingao.yang, 2014.9.24 */

	bm_print(BM_LOG_CRTI, "[allan add ] vol_bat=%d, gFG_voltage=%d, \n",vol_bat,gFG_voltage);
	gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage);
	vbat_capacity = fgauge_read_capacity_by_v(vol_bat);
	bm_print(BM_LOG_CRTI, "[allan add ] gFG_capacity_by_v=%d, vbat_capacity=%d, \n",gFG_capacity_by_v,vbat_capacity);
...
void oam_run(void)
{
    ...
    bm_print(BM_LOG_FULL, "[oam_run] %d,%d,%d,%d,%d,%d,%d,%d\n",
    ...
    bm_print(BM_LOG_FULL, "[oam_total_s] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
        gFG_capacity_by_c,        // 1
        gFG_capacity_by_v,        // 2
	...
	bm_print(BM_LOG_FULL, "[oam_init_inf] %d, %d, %d, %d, %d, %d\n",
		gFG_voltage, (100 - fgauge_read_capacity_by_v(gFG_voltage)), g_rtc_fg_soc, gFG_DOD0 ,oam_v_ocv_init, force_get_tbat());

	 bm_print(BM_LOG_FULL, "[oam_run_inf] %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
...
#ifdef BATTERY_COMPATIBLE	/* allan add start on 20131122 */
void battery_meter_check_bat_id(void)
{
    int ret=0;
    int val=0;
    
    val = 5; // set avg times
    ret = read_adc_v_bat_id(&val);

	bm_print(BM_LOG_CRTI, " **************************************** \n", val);
	bm_print(BM_LOG_CRTI, "battery_meter_check_bat_id: bat_id = %d\n", val);
	bm_print(BM_LOG_CRTI, " **************************************** \n", val);

	bat_id_vol = val;
#ifdef POP7_BATTERY_COMPATIBLE	
    if(val >= BAT_FEIMT_POP7_VALUE)
		battery_compatible = BAT_FEIMT_POP7;
	else if ((val >= BAT_OTHER_POP7_VALUE)&&(val < BAT_MBELL_POP7_VALUE))
		battery_compatible = BAT_MBELL_POP7;
	else
		battery_compatible = BAT_OTHER_POP7;
#else
	if(val >= BAT_FEIMT_POP8_VALUE)
		battery_compatible = BAT_FEIMT_POP8;
	else
		battery_compatible = BAT_OTHER_POP8;
#endif
}
#endif	/* allan add end on 20131122 */
...
/* [PLATFORM]-Add-BEGIN by TCTSZ.pingao.yang, 2014.9.3, refer pr 781992 */
	ICharging = (ICharging * CURRENT_ADJUST_MUL) / CURRENT_ADJUST_DIV;
/* [PLATFORM]-Add-END by TCTSZ.pingao.yang, 2014.9.3 */
...
static int battery_meter_suspend(struct platform_device *dev, pm_message_t state)    
{
	...
	if (_g_bat_sleep_total_time < g_spm_timer) {
		//return 0;	/*allan modify on 20140805 */
	}
...
static int battery_meter_resume(struct platform_device *dev)
{
	...
	//printk("<2>""%s sleep time = %d, g_spm_timer = %d\n",
	//		__func__, _g_bat_sleep_total_time, g_spm_timer);
#if 0	/*[PLATFORM]-Add-BEGIN by TCTSZ.pingao.yang, 2014.08.20, refer PR 768703 */
	if (_g_bat_sleep_total_time < g_spm_timer) {
		//return 0;	 /*allan modify on 20140805 */
	}
...
#endif	/*[PLATFORM]-Add-BEGIN by TCTSZ.pingao.yang, 2014.08.20 */
...
#endif        
    {
    	/*[PLATFORM]-Add-BEGIN by TCTSZ.pingao.yang, 2014.08.20, refer PR 768703 */
		...
		/*[PLATFORM]-Add-BEGIN by TCTSZ.pingao.yang, 2014.08.20.*/
		...
		if((rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec) > 3600)
    	{
    		...
    		else
    		{
				 ...
				 oam_car_delta = suspend_current* (rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec + suspend_oam_time)/3600;
				 suspend_oam_time = suspend_current* (rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec + suspend_oam_time)%3600;
