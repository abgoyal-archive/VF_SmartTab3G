...
/* allan add on 20140219 */
unsigned int system_off_count = 0;	
unsigned int fual_ui_hold_count = 0;
INT32 pre_ui_soc = 0;
INT32 charger_second_check = 0;
kal_int32 cur_timer_counter = 0;
kal_int32 pre_timer_counter = 0;
kal_int32 sleep_timer = 0;
kal_int32 soc_reduce_step = 0;
kal_int32 system_off_timer = 0;
extern unsigned long rtc_read_hw_time(void);
#define STANDARD_HOST_CURRENT 490
/* allan add end  20140219 */
...
int sys_chg_control; /* allan add on 20131105 */
    ...
    union power_supply_propval *val)
{
    ...
    battery_xlog_printk(BAT_LOG_FULL, "[wireless_get_property] start\n");
    switch (psp) {
    case POWER_SUPPLY_PROP_ONLINE:                           
        val->intval = data->WIRELESS_ONLINE;
        battery_xlog_printk(BAT_LOG_FULL, "[wireless_get_property] data->WIRELESS_ONLINE %d\n", data->WIRELESS_ONLINE);
        break;
    default:
        ret = -EINVAL;
        break;
...
//del by fangyou.wang, for sometimes can't boot up
#if 0
    if(data->BAT_CAPACITY == 50)
    {
        mt_battery_GetBatteryData();
        data->BAT_CAPACITY = BMT_status.SOC;
        battery_xlog_printk(BAT_LOG_FULL, "battery_get_property batteryCapacity not init\r\n");
    }
#endif
    switch (psp) {
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	#if 0	//ifdef MTK_DISABLE_POWER_ON_OFF_VOLTAGE_LIMITATION	//allan modify
...
static kal_bool mt_battery_0Percent_tracking_check(void)
{
	...
	system_off_timer = cur_timer_counter - pre_timer_counter;
    ...
    else
    {
		...
		if (system_off_timer <= 80)	//80 s
	    {
	    	system_off_count++;		/* allan add on 20131120 */
			if((system_off_count >= SYSTEM_OFF_PREVENT_COUNT) || (fual_ui_hold_count >= FUAL_UI_HOLD_COUNT))
			{
				system_off_count = 0;
	    		BMT_status.UI_SOC--;
			}	
...
static void battery_update(struct battery_data *bat_data)
{
    ...
    {
    	if(pre_ui_soc == BMT_status.UI_SOC)	/* allan add on 20131120 */
    	{
			fual_ui_hold_count++;
    	}
		else
		{
			pre_ui_soc = BMT_status.UI_SOC;
			fual_ui_hold_count = 0;
		}
	...
	if ((BMT_status.charger_exist != KAL_TRUE) && (BMT_status.bat_vol <= SYSTEM_OFF_VOLTAGE) 
		 && (BMT_status.UI_SOC <= 1) 
		 && ((system_off_count >= SYSTEM_OFF_UI_COUNT) || ((pre_timer_counter - cur_timer_counter) > SYSTEM_OFF_SLEEP_TIME))) 
	{
		BMT_status.UI_SOC = 0;
	}

	if ((BMT_status.charger_exist != KAL_TRUE) && (BMT_status.bat_vol <= SYSTEM_POWER_OFF_VOLTAGE))
	{
		BMT_status.UI_SOC = 0;
	}
    ...
    if ((BMT_status.UI_SOC == 1) ||(BMT_status.bat_vol <= SYSTEM_OFF_VOLTAGE)){
...
static ssize_t chg_enable_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	return sprintf(buf, "%d\n", !sys_chg_control);
}	

static DEVICE_ATTR(chg_enable, 0644, chg_enable_show, chg_enable_store);
/* allan add end on 20131105 */ 
///////////////////////////////////////////////////////////////////////////////////////////
...
{ 
	...
	static kal_int32 ICharging_prev = 0;
	...
	if((BMT_status.charger_type == STANDARD_HOST) && (ICharging >= STANDARD_HOST_CURRENT))
		ICharging = STANDARD_HOST_CURRENT;
	/* [PLATFORM]-Add-END by TCTSZ.pingao.yang, 2014.9.3 */

	/* [PLATFORM]-Add-BEGIN by TCTSZ.pingao.yang, 2014.9.24,  real time update charging current */
	if(abs(ICharging_prev - ICharging) < ICHARGING_DIFF)
	{
		ICharging = ICharging_prev;
	}
	ICharging_prev = ICharging;
...
static void mt_battery_notify_TatalChargingTime_check(void)
{
    ...
    battery_xlog_printk(BAT_LOG_FULL, "[BATTERY] BATTERY_NOTIFY_CASE_0005_TOTAL_CHARGINGTIME (%x)\n", g_BatteryNotifyCode);
#endif
}
...
static void mt_battery_notify_ICharging_check(void)
{
    ...
    battery_xlog_printk(BAT_LOG_FULL, "[BATTERY] BATTERY_NOTIFY_CASE_0003_ICHARGING (%x)\n", g_BatteryNotifyCode);
...
static void mt_battery_notify_UI_test(void)
{
    ...
    {
        g_BatteryNotifyCode = 0x0001;
        battery_xlog_printk(BAT_LOG_FULL, "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0001_VCHARGER\n");
    }
    else if(g_BN_TestMode == 0x0002)
    {
        g_BatteryNotifyCode = 0x0002;
        battery_xlog_printk(BAT_LOG_FULL, "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0002_VBATTEMP\n");
    }
    else if(g_BN_TestMode == 0x0003)
    {
        g_BatteryNotifyCode = 0x0004;
        battery_xlog_printk(BAT_LOG_FULL, "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0003_ICHARGING\n");
    }
    else if(g_BN_TestMode == 0x0004)
    {
        g_BatteryNotifyCode = 0x0008;
        battery_xlog_printk(BAT_LOG_FULL, "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0004_VBAT\n");
    }
    else if(g_BN_TestMode == 0x0005)
    {
        g_BatteryNotifyCode = 0x0010;
        battery_xlog_printk(BAT_LOG_FULL, "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0005_TOTAL_CHARGINGTIME\n");
    }
    else
    {
        battery_xlog_printk(BAT_LOG_FULL, "[BATTERY] Unknown BN_TestMode Code : %x\n", g_BN_TestMode);
    }
}
...
static void mt_battery_charger_detect_check(void)
{
    if( upmu_is_chr_det() == KAL_TRUE )
    {
		...
		if(BMT_status.charger_type == CHARGER_UNKNOWN)
		{
			...
			/* allan add start on 20131224 */
			charger_second_check = 0;	
			while((BMT_status.charger_type == NONSTANDARD_CHARGER) && (charger_second_check <= 40))
			{
				charger_second_check++; 
				mdelay(50);
				mt_charger_type_detection();
				//battery_xlog_printk(BAT_LOG_CRTI, "[BAT_thread] 2nd, CHR_Type_num=%d\r\n", BMT_status.charger_type);
			}
			/* allan add start end 20131224 */
			if((BMT_status.charger_type==STANDARD_HOST) || (BMT_status.charger_type==CHARGING_HOST) )
	        	{
	           		mt_usb_connect();
			}
...
static int battery_probe(struct platform_device *dev)    
{
    ...
    /* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_chg_enable); /* allan add on 20131105 */  
