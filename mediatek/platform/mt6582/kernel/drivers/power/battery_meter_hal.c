...
kal_int32 read_adc_v_bat_id(void *data)
{
	int ret = 0, data0[4], i, ret_value = 0, ret_temp = 0;
    int Channel = 1;
	int times = 6;

	if(IMM_IsAdcInitReady() == 0)
	{
        bm_print(BM_LOG_CRTI, "[get_bat_id_volt] AUXADC is not ready");
        return 0;
    }
    ...
    while (i--)
    {
        ret_value = IMM_GetOneChannelValue(Channel, data0, &ret_temp);
        ret += ret_temp;
        bm_print(BM_LOG_CRTI, "[get_bat_id_volt] %d\n",ret_temp);
    }
    
    ret = ret*1500/4096 ;
    ret = ret/times;
    bm_print(BM_LOG_CRTI, "[get_bat_id_volt] Battery output mV = %d\n",ret);
