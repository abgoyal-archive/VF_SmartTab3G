...
/*[PLATFORM]-Add-BEGIN by TCTSZ.huangjing,2014.06.26,set min duty ratio to 5% ,
	and do not allow third party software to shutdown backlight.
*/
extern int LED_ON_OFF_FLAG;
...
static int charging_current_limit_flag = false;	
...
int disp_bls_set_backlight(unsigned int level)
{
	...
	if (level < 100)
		charging_current_limit_flag = false;
	else
		charging_current_limit_flag = true;
...
int get_lcd_backlight_on_status(void)
{
	//DISP_MSG("get_lcd_backlight_on_status: charging_current_limit_flag = %d\n", charging_current_limit_flag);
	return charging_current_limit_flag;
}
