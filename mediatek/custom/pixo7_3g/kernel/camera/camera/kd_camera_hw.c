...
#define PK_XLOG_INFO(fmt, args...)
#endif
/*****added flag by jhuang for main and sub sensor ******/
int glob_SensorIdx=-1;  
/*****added flag by jhuang for main and sub sensor ******/
...
/*****added flag by jhuang for main and sub sensor jinghuang@tcl.com******/
 glob_SensorIdx=-1; 
/*****added flag by jhuang for main and sub sensor jinghuang@tcl.com******/
...
u32 pinSet[2][8] = {
	...
	//We ware suggested to set gpio off before power on
	if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
       ...
       // mdelay(10);
			...
			if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1200,mode_name))
        {
            ...
            if(pinSetIdx == 0 && TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1800,mode_name))
        {
				PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
             //return -EIO;
             //goto _kdCISModulePowerOn_exit_;
	...
	if(mt_set_gpio_out(pinSet[1-pinSetIdx][IDX_PS_CMRST],pinSet[1-pinSetIdx][IDX_PS_CMRST+IDX_PS_ON])){PK_DBG("[CAMERA SENSOR] set CMRST ON failed!! \n");} //low == reset sensor
	mdelay(10);
	if(mt_set_gpio_out(pinSet[1-pinSetIdx][IDX_PS_CMRST],pinSet[1-pinSetIdx][IDX_PS_CMRST+IDX_PS_OFF])){PK_DBG("[CAMERA SENSOR] set CMRST OFF failed!! \n");} //low == reset sensor
	mdelay(10);
	if(mt_set_gpio_out(pinSet[1-pinSetIdx][IDX_PS_CMRST],pinSet[1-pinSetIdx][IDX_PS_CMRST+IDX_PS_ON])){PK_DBG("[CAMERA SENSOR] set CMRST ON failed!! \n");} //low == reset sensor
	mdelay(10);
	if(mt_set_gpio_out(pinSet[1-pinSetIdx][IDX_PS_CMPDN],pinSet[1-pinSetIdx][IDX_PS_CMPDN+IDX_PS_OFF])){PK_DBG("[CAMERA LENS] set gpio failed!! \n");} //high == power down lens module
	mdelay(10);
       ...
       // PK_DBG("[OFF]sensorIdx:%d \n",SensorIdx);
        if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
			...
			mt_set_gpio_mode(GPIO1,GPIO_MODE_00);
