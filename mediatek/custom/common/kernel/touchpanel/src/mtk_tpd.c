...
static struct proc_dir_entry *tp_dir,*tp_device_name;
...
static int tp_device_read_proc(char * page,char * * start,off_t off,int count,int * eof,void * data)
{
    int len = 0;
	#ifdef PIXO_7_3G
	if((g_tpd_drv == NULL)||(g_tpd_drv->tpd_device_name == NULL))
	{
      len += sprintf(page, "unknow \n");
	}
	  else
	{
...
static int tpd_probe(struct platform_device *pdev) {
  ...
  TPD_RES_X = DISP_GetScreenWidth();
    ...
    tp_device_name = create_proc_read_entry( "tp_device", 0666, tp_dir, tp_device_read_proc, NULL);
