...
PRODUCT_COPY_FILES += $(MTK_ROOT_CONFIG_OUT)/mtk-kpd.kl:system/usr/keylayout/mtk-kpd.kl \
                      ...
                      $(MTK_ROOT_CONFIG_OUT)/peel_config:system/etc/peel_config \
                      $(MTK_ROOT_CONFIG_OUT)/peel_config:custpack/peel_config \
                      $(MTK_ROOT_CONFIG_OUT)/PR1662965.img:system/etc/firmware/PR1662965.img 
...
# added by mingzhi.shen, compile phone module when TCL_TELEPHONY_SUPPORT=true, BEGIN
ifneq ($(TCL_TABLEET_FEATURE_SWITCH_OPTION),WIFI_ONLY)
ifneq ($(TCL_TABLEET_FEATURE_SWITCH_OPTION),3GDATA_SMS)
ifneq ($(TCL_TABLEET_FEATURE_SWITCH_OPTION),3GDATA_ONLY)
...
PRODUCT_COPY_FILES += $(filter-out %telephony.gsm.xml,$(strip \
                        $(foreach file,$(wildcard $(MTK_ROOT_CONFIG_OUT)/*.xml), \
                          $(addprefix $(MTK_ROOT_CONFIG_OUT)/$(notdir $(file)):system/etc/permissions/,$(notdir $(file))) \
                         ) \
                       ))
...
PRODUCT_COPY_FILES += \
                      $(MTK_ROOT_CONFIG_OUT)/init.modem.empty.rc:root/init.modem.rc \
                      $(MTK_ROOT_CONFIG_OUT)/meta_init.modem.empty.rc:root/meta_init.modem.rc
else
PRODUCT_COPY_FILES += \
...
# added by mingzhi.shen, END

# added by mingzhi.shen, for ir control, BEGIN
ifeq ($(TCL_CUSTOMER_CONFIG),VF)
PRODUCT_COPY_FILES := $(filter-out %android.hardware.consumerir.xml, $(PRODUCT_COPY_FILES))
endif
# added by mingzhi.shen, END
